...
@RestController
@RequestMapping({"/gruposinvestigacion"})

public class GruposInvestigacionController {

    @Autowired
    GruposInvestigacionService service;

    private final Logger LOG = LoggerFactory.getLogger(GruposInvestigacionController.class);
    ...
    @PostMapping
    public ResponseEntity agregar(@RequestBody GruposInvestigacion g) {
        try {
            return ResponseEntity.ok(new ResponseDto(service.add(g), true));
        } catch (Exception e) {
            LOG.error("No se pudo completar ", e);
            return ResponseEntity.internalServerError()
                    .body(new ResponseDto("No se pudo completar", false));
    ...
    @PutMapping(path = {"/{id}"})
    public ResponseEntity editar(@RequestBody GruposInvestigacion g, @PathVariable("id") int id) {
        try {
            g.setId(id);
            return ResponseEntity.ok(new ResponseDto(service.edit(g), true));
        } catch (Exception e) {
            LOG.error("No se pudo completar ", e);
            return ResponseEntity.internalServerError()
                    .body(new ResponseDto("No se pudo completar", false));
