...
public class CustomersController {
    ...
    @PostMapping
    public ResponseEntity agregar(@RequestBody Customers c) {
        try {
    ...
    @PutMapping(path = {"/{id}"})
    public ResponseEntity editar(@RequestBody Customers c, @PathVariable("id") int id) {
        try {
            c.setId(id);
    ...
    public ResponseEntity listarByIdPerson(@PathVariable("id_person") int id_person) {
        try {
            return ResponseEntity.ok(new ResponseDto(service.listarByIdPerson(id_person), true));
        } catch (Exception e) {
            LOG.error("No se pudo completar ", e);
            return ResponseEntity.internalServerError()
                    .body(new ResponseDto("No se pudo completar", false));
    ...
    @GetMapping(path = {"/findByZone/{id_zone}"})
    public ResponseEntity listarByIdZone(@PathVariable("id_zone") int id_zone) {
        try {
            return ResponseEntity.ok(new ResponseDto(service.listarByIdZone(id_zone), true));
        } catch (Exception e) {
            LOG.error("No se pudo completar ", e);
            return ResponseEntity.internalServerError()
                    .body(new ResponseDto("No se pudo completar", false));
