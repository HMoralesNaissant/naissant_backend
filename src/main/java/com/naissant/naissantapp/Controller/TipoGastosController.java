...
public class TipoGastosController {
    ...
    @Autowired
    TipoGastosService service;

    private final Logger LOG = LoggerFactory.getLogger(TipoGastosController.class);
    ...
    @PostMapping
    public ResponseEntity agregar(@RequestBody TipoGastos t) {
        try {
    ...
    @PutMapping(path = {"/{id}"})
    public ResponseEntity editar(@RequestBody TipoGastos t, @PathVariable("id") int id) {
        try {
            t.setId(id);
