...
public class ProfilesController {
    ...
    private final Logger LOG = LoggerFactory.getLogger(ProfilesController.class);
    ...
    @PostMapping
    public ResponseEntity agregar(@RequestBody Profiles p) {
        try {
    ...
    @PutMapping(path = {"/{id}"})
    public ResponseEntity editar(@RequestBody Profiles p, @PathVariable("id") int id) {
        try {
            p.setId(id);
    ...
    public ResponseEntity listarByDescription(@PathVariable("description") String description) {
        try {
            return ResponseEntity.ok(new ResponseDto(service.listarByDescription(description), true));
        } catch (Exception e) {
            LOG.error("No se pudo completar ", e);
            return ResponseEntity.internalServerError()
                    .body(new ResponseDto("No se pudo completar", false));
    ...
    public ResponseEntity delete(@PathVariable("id") int id){
        try{
            service.delete(id);
            return ResponseEntity.ok(new ResponseDto("OK", true));
        } catch (Exception e) {
            LOG.error("No se pudo completar ", e);
            return ResponseEntity.internalServerError()
                    .body(new ResponseDto("No se pudo completar", false));
