...
@RestController
@RequestMapping({"/pensionfund"})
@CrossOrigin(origins = "http://localhost:4200", maxAge = 3600)

public class PensionFundController {

    @Autowired
    PensionFundService service;

    private final Logger LOG = LoggerFactory.getLogger(PensionFundController.class);
    ...
    @PostMapping
    public ResponseEntity agregar(@RequestBody PensionFund p) {
        try {
    ...
    @PutMapping(path = {"/{id}"})
    public ResponseEntity editar(@RequestBody PensionFund p, @PathVariable("id") int id) {
        try {
            p.setId(id);
