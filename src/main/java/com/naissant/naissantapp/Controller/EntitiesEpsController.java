...
@RestController
@RequestMapping({"/entitieseps"})
@CrossOrigin(origins = "http://localhost:4200", maxAge = 3600)

public class EntitiesEpsController {

    @Autowired
    EntitiesEpsService service;

    private final Logger LOG = LoggerFactory.getLogger(EntitiesEpsController.class);
    ...
    @PostMapping
    public ResponseEntity agregar(@RequestBody EntitiesEps p) {
        try {
    ...
    @PutMapping(path = {"/{id}"})
    public ResponseEntity editar(@RequestBody EntitiesEps p, @PathVariable("id") int id) {
        try {
            p.setId(id);
