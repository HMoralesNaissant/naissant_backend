...
@RestController
@RequestMapping({"/portfolioage"})
@CrossOrigin(origins = "http://localhost:4200", maxAge = 3600)

public class PortfolioAgeController {

    @Autowired
    PortfolioAgeService service;

    private final Logger LOG = LoggerFactory.getLogger(PortfolioAgeController.class);
    ...
    @PostMapping
    public ResponseEntity agregar(@RequestBody PortfolioAge p) {
        try {
    ...
    @PutMapping(path = {"/{id}"})
    public ResponseEntity editar(@RequestBody PortfolioAge p, @PathVariable("id") int id) {
        try {
            p.setId(id);
