...
@RestController
@RequestMapping({"/anosemestre"})

public class AnoSemestreController {

    @Autowired
    AnoSemestreService service;
    
    private final Logger LOG = LoggerFactory.getLogger(AnoSemestreController.class);
    ...
    @PostMapping
    public ResponseEntity agregar(@RequestBody AnoSemestre a) {
        try {
    ...
    @PutMapping(path = {"/{id}"})
    public ResponseEntity editar(@RequestBody AnoSemestre a, @PathVariable("id") int id) {
        try {
            a.setId(id);
