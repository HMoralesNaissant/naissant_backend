...
@RestController
@RequestMapping({"/tipoprogramas"})

public class TipoProgramasController {

    @Autowired
    TipoProgramasService service;

    private final Logger LOG = LoggerFactory.getLogger(TipoProgramasController.class);
    ...
    @PostMapping
    public ResponseEntity agregar(@RequestBody TipoProgramas t) {
        try {
    ...
    @PutMapping(path = {"/{id}"})
    public ResponseEntity editar(@RequestBody TipoProgramas t, @PathVariable("id") int id) {
        try {
            t.setId(id);
