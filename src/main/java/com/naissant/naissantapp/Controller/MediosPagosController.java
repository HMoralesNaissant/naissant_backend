...
@RestController
@RequestMapping({"/mediospagos"})

public class MediosPagosController {

    @Autowired
    MediosPagosService service;

    private final Logger LOG = LoggerFactory.getLogger(MediosPagosController.class);
    ...
    @PostMapping
    public ResponseEntity agregar(@RequestBody MediosPagos m) {
        try {
    ...
    @PutMapping(path = {"/{id}"})
    public ResponseEntity editar(@RequestBody MediosPagos m, @PathVariable("id") int id) {
        try {
            m.setId(id);
