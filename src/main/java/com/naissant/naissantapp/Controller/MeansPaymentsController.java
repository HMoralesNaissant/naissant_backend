...
public class MeansPaymentsController {

    @Autowired
    MeansPaymentsService service;

    private final Logger LOG = LoggerFactory.getLogger(MeansPaymentsController.class);
    ...
    @PostMapping
    public ResponseEntity agregar(@RequestBody MeansPayments m) {
        try {
    ...
    @PutMapping(path = {"/{id}"})
    public ResponseEntity editar(@RequestBody MeansPayments m, @PathVariable("id") int id) {
        try {
            m.setId(id);
