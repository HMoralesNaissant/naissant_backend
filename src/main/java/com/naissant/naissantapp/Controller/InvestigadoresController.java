...
public class InvestigadoresController {
    ...
    @Autowired
    InvestigadoresService service;

    private final Logger LOG = LoggerFactory.getLogger(InvestigadoresController.class);
    ...
    @PostMapping
    public ResponseEntity agregar(@RequestBody Investigadores i) {
        try {
    ...
    @PutMapping(path = {"/{id}"})
    public ResponseEntity editar(@RequestBody Investigadores i, @PathVariable("id") int id) {
        try {
            i.setId(id);
