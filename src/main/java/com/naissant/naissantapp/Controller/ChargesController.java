...
public class ChargesController {
    ...
    private final Logger LOG = LoggerFactory.getLogger(ChargesController.class);
    ...
    @PostMapping
    public ResponseEntity agregar(@RequestBody Charges c) {
        try {
    ...
    @PutMapping(path = {"/{id}"})
    public ResponseEntity editar(@RequestBody Charges c, @PathVariable("id") int id) {
        try {
            c.setId(id);
    ...
    @GetMapping(path = {"/findByAreas/{id_areas}"})
    public ResponseEntity listarByIdAreas(@PathVariable("id_areas") int id_areas) {
        try {
            return ResponseEntity.ok(new ResponseDto(service.listarByIdAreas(id_areas), true));
        } catch (Exception e) {
            LOG.error("No se pudo completar ", e);
            return ResponseEntity.internalServerError()
                    .body(new ResponseDto("No se pudo completar", false));
