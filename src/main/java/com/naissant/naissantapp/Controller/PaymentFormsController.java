...
@RestController
@RequestMapping({"/paymentforms"})
@CrossOrigin(origins = "http://localhost:4200", maxAge = 3600)

public class PaymentFormsController {

    @Autowired
    PaymentFormsService service;

    private final Logger LOG = LoggerFactory.getLogger(PaymentFormsController.class);
    ...
    @PostMapping
    public ResponseEntity agregar(@RequestBody PaymentForms f) {
        try {
    ...
    @PutMapping(path = {"/{id}"})
    public ResponseEntity editar(@RequestBody PaymentForms f, @PathVariable("id") int id) {
        try {
            f.setId(id);
