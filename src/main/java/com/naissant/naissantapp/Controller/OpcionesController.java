...
public class OpcionesController {
    ...
    @Autowired
    OpcionesService service;

    private final Logger LOG = LoggerFactory.getLogger(OpcionesController.class);
    ...
    @PostMapping
    public ResponseEntity agregar(@RequestBody Opciones o) {
        try {
    ...
    @PutMapping(path = {"/{id}"})
    public ResponseEntity editar(@RequestBody Opciones o, @PathVariable("id") int id) {
        try {
            o.setId(id);
    ...
    @GetMapping(path = {"/findByModulo/{id_modulo}"})
    public ResponseEntity listarByIdModulo(@PathVariable("id_modulo") int id_modulo) {
        try {
            return ResponseEntity.ok(new ResponseDto(service.listarByIdModulo(id_modulo), true));
        } catch (Exception e) {
            LOG.error("No se pudo completar ", e);
            return ResponseEntity.internalServerError()
                    .body(new ResponseDto("No se pudo completar", false));
