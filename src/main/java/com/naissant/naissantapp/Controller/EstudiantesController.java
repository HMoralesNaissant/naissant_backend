...
public class EstudiantesController {
    ...
    @PostMapping
    public ResponseEntity agregar(@RequestBody Estudiantes u) {
        try {
    ...
    @PutMapping(path = {"/{id}"})
    public ResponseEntity editar(@RequestBody Estudiantes u, @PathVariable("id") int id) {
        try {
            u.setId(id);
    ...
    @GetMapping(path = {"/findBySalutem/{salutem}"})
    public ResponseEntity listarBySalutem(@PathVariable("salutem") String salutem) {
        try {
            return ResponseEntity.ok(new ResponseDto(service.listarBySalutem(salutem), true));
        } catch (Exception e) {
            LOG.error("No se pudo completar ", e);
            return ResponseEntity.internalServerError()
                    .body(new ResponseDto("No se pudo completar", false));
