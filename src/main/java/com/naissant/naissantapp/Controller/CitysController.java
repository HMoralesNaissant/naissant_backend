...
public class CitysController {
    ...
    private final Logger LOG = LoggerFactory.getLogger(CitysController.class);
    ...
    @PostMapping
    public ResponseEntity agregar(@RequestBody Citys c) {
        try {
    ...
    @PutMapping(path = {"/{id}"})
    public ResponseEntity editar(@RequestBody Citys c, @PathVariable("id") int id) {
        try {
            c.setId(id);
    ...
    @GetMapping(path = {"/findByDepartments/{id_dpts}"})
    public ResponseEntity listarByIdDpts(@PathVariable("id_dpts") int id_dpts) {
        try {
            return ResponseEntity.ok(new ResponseDto(service.listarByIdDpts(id_dpts), true));
        } catch (Exception e) {
            LOG.error("No se pudo completar ", e);
            return ResponseEntity.internalServerError()
                    .body(new ResponseDto("No se pudo completar", false));
