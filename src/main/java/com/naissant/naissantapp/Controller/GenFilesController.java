...
@RestController
@RequestMapping({"/genfiles"})
@CrossOrigin(origins = "http://localhost:4200", maxAge = 3600)

public class GenFilesController {

    @Autowired
    GenFilesService service;

    private final Logger LOG = LoggerFactory.getLogger(GenFilesController.class);
    ...
    @PostMapping
    public ResponseEntity agregar(@RequestBody GenFiles u) {
        try {
    ...
    @PutMapping(path = {"/{id}"})
    public ResponseEntity editar(@RequestBody GenFiles u, @PathVariable("id") int id) {
        try {
            u.setId(id);
