...
public class OptionsController {
    ...
    private final Logger LOG = LoggerFactory.getLogger(OptionsController.class);
    ...
    @PostMapping
    public ResponseEntity agregar(@RequestBody Options o) {
        try {
    ...
    @PutMapping(path = {"/{id}"})
    public ResponseEntity editar(@RequestBody Options o, @PathVariable("id") int id) {
        try {
            o.setId(id);
    ...
    @GetMapping(path = {"/findByModule/{id_module}"})
    public ResponseEntity listarByIdModule(@PathVariable("id_module") int id_module) {
        try {
            return ResponseEntity.ok(new ResponseDto(service.listarByIdModule(id_module), true));
        } catch (Exception e) {
            LOG.error("No se pudo completar ", e);
            return ResponseEntity.internalServerError()
                    .body(new ResponseDto("No se pudo completar", false));
