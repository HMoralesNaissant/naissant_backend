...
@RestController
@RequestMapping({"/entitiesccf"})
@CrossOrigin(origins = "http://localhost:4200", maxAge = 3600)

public class EntitiesCcfController {

    @Autowired
    EntitiesCcfService service;

    private final Logger LOG = LoggerFactory.getLogger(EntitiesCcfController.class);
    ...
    @PostMapping
    public ResponseEntity agregar(@RequestBody EntitiesCcf c) {
        try {
    ...
    @PutMapping(path = {"/{id}"})
    public ResponseEntity editar(@RequestBody EntitiesCcf c, @PathVariable("id") int id) {
        try {
            c.setId(id);
