...
@RestController
@RequestMapping({"/entitiestemporary"})
@CrossOrigin(origins = "http://localhost:4200", maxAge = 3600)

public class EntitiesTemporaryController {

    @Autowired
    EntitiesTemporaryService service;

    private final Logger LOG = LoggerFactory.getLogger(EntitiesTemporaryController.class);
    ...
    @PostMapping
    public ResponseEntity agregar(@RequestBody EntitiesTemporary t) {
        try {
    ...
    @PutMapping(path = {"/{id}"})
    public ResponseEntity editar(@RequestBody EntitiesTemporary t, @PathVariable("id") int id) {
        try {
            t.setId(id);
