...
public class CatProductsController {

    @Autowired
    CatProductsService service;

    private final Logger LOG = LoggerFactory.getLogger(CatProductsController.class);
    ...
    @PostMapping
    public ResponseEntity agregar(@RequestBody CatProducts p) {
        try {
    ...
    @PutMapping(path = {"/{id}"})
    public ResponseEntity editar(@RequestBody CatProducts p, @PathVariable("id") int id) {
        try {
            p.setId(id);
