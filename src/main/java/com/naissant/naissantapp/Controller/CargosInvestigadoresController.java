...
@RestController
@RequestMapping({"/cargosinvestigadores"})

public class CargosInvestigadoresController {

    @Autowired
    CargosInvestigadoresService service;

    private final Logger LOG = LoggerFactory.getLogger(CargosInvestigadoresController.class);
    ...
    @PostMapping
    public ResponseEntity agregar(@RequestBody CargosInvestigadores c) {
        try {
    ...
    @PutMapping(path = {"/{id}"})
    public ResponseEntity editar(@RequestBody CargosInvestigadores c, @PathVariable("id") int id) {
        try {
            c.setId(id);
