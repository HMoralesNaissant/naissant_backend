...
@RestController
@RequestMapping({"/entitiesarl"})
@CrossOrigin(origins = "http://localhost:4200", maxAge = 3600)

public class EntitiesArlController {

    @Autowired
    EntitiesArlService service;

    private final Logger LOG = LoggerFactory.getLogger(EntitiesArlController.class);
    ...
    @PostMapping
    public ResponseEntity agregar(@RequestBody EntitiesArl a) {
        try {
    ...
    @PutMapping(path = {"/{id}"})
    public ResponseEntity editar(@RequestBody EntitiesArl a, @PathVariable("id") int id) {
        try {
            a.setId(id);
