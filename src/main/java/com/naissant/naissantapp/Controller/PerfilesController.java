...
public class PerfilesController {
    ...
    private final Logger LOG = LoggerFactory.getLogger(PerfilesController.class);
    ...
    @PostMapping
    public ResponseEntity agregar(@RequestBody Perfiles p) {
        try {
    ...
    @PutMapping(path = {"/{id}"})
    public ResponseEntity editar(@RequestBody Perfiles p, @PathVariable("id") int id) {
        try {
            p.setId(id);
    ...
    public ResponseEntity listarByNombre(@PathVariable("nombre") String nombre) {
        try {
            return ResponseEntity.ok(new ResponseDto(service.listarByNombre(nombre), true));
        } catch (Exception e) {
            LOG.error("No se pudo completar ", e);
            return ResponseEntity.internalServerError()
                    .body(new ResponseDto("No se pudo completar", false));
