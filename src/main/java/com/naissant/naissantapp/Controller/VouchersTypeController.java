...
@RestController
@RequestMapping({"/voucherstype"})
@CrossOrigin(origins = "http://localhost:4200", maxAge = 3600)

public class VouchersTypeController {

    @Autowired
    VouchersTypeService service;

    private final Logger LOG = LoggerFactory.getLogger(VouchersTypeController.class);
    ...
    @PostMapping
    public ResponseEntity agregar(@RequestBody VouchersType v) {
        try {
    ...
    @PutMapping(path = {"/{id}"})
    public ResponseEntity editar(@RequestBody VouchersType v, @PathVariable("id") int id) {
        try {
            v.setId(id);
