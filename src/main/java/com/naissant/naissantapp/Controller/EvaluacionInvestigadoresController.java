...
@RestController
@RequestMapping({"/evaluacioninvestigadores"})

public class EvaluacionInvestigadoresController {

    @Autowired
    EvaluacionInvestigadoresService service;

    private final Logger LOG = LoggerFactory.getLogger(EvaluacionInvestigadoresController.class);
    ...
    @PostMapping
    public ResponseEntity agregar(@RequestBody EvaluacionInvestigadores u) {
        try {
    ...
    @PutMapping(path = {"/{id}"})
    public ResponseEntity editar(@RequestBody EvaluacionInvestigadores u, @PathVariable("id") int id) {
        try {
            u.setId(id);
