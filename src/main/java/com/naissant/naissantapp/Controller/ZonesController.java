...
public class ZonesController {
    ...
    private final Logger LOG = LoggerFactory.getLogger(ZonesController.class);
    ...
    @PostMapping
    public ResponseEntity agregar(@RequestBody Zones z) {
        try {
            return ResponseEntity.ok(new ResponseDto(service.add(z), true));
        } catch (Exception e) {
            LOG.error("No se pudo completar ", e);
            return ResponseEntity.internalServerError()
                    .body(new ResponseDto("No se pudo completar", false));
    ...
    @PutMapping(path = {"/{id}"})
    public ResponseEntity editar(@RequestBody Zones z, @PathVariable("id") int id) {
        try {
            z.setId(id);
            return ResponseEntity.ok(new ResponseDto(service.edit(z), true));
        } catch (Exception e) {
            LOG.error("No se pudo completar ", e);
            return ResponseEntity.internalServerError()
                    .body(new ResponseDto("No se pudo completar", false));
