...
@RestController
@RequestMapping({"/saleshours"})
@CrossOrigin(origins = "http://localhost:4200", maxAge = 3600)

public class SalesHoursController {

    @Autowired
    SalesHoursService service;

    private final Logger LOG = LoggerFactory.getLogger(SalesHoursController.class);
    ...
    @PostMapping
    public ResponseEntity agregar(@RequestBody SalesHours s) {
        try {
    ...
    @PutMapping(path = {"/{id}"})
    public ResponseEntity editar(@RequestBody SalesHours s, @PathVariable("id") int id) {
        try {
            s.setId(id);
