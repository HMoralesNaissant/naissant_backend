...
public class ProyectosController {
    ...
    @PostMapping
    public ResponseEntity agregar(@RequestBody Proyectos p) {
        try {
    ...
    @PutMapping(path = {"/{id}"})
    public ResponseEntity editar(@RequestBody Proyectos p, @PathVariable("id") int id) {
        try {
            p.setId(id);
    ...
    @PostMapping(path= "/{proyecto_id}/file/upload", consumes = {MediaType.MULTIPART_FORM_DATA_VALUE})
    public ResponseEntity uploadFiles(
            @RequestParam("files") MultipartFile[] files,
            @PathVariable(name = "proyecto_id") Integer proyectoId,
            @RequestParam("descripcion") String descripcion) {
        
        try {
            return ResponseEntity.ok().body(service.saveArchivo(proyectoId, files, descripcion));
        } catch (Exception e) {
            LOG.error("No se pudo completar ", e);
            return ResponseEntity.internalServerError()
    ...
    @GetMapping("/{proyecto_id}/file")
    public ResponseEntity getFile(@PathVariable(name = "proyecto_id") Integer proyectoId) {
        try {
            Resource file = service.downloadFile(proyectoId);
