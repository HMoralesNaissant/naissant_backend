...
@RestController
@RequestMapping({"/cuentasxcobrardet"})

public class CuentasxcobrarDetController {

    @Autowired
    CuentasxcobrarDetService service;

    private final Logger LOG = LoggerFactory.getLogger(CuentasxcobrarDetController.class);
    ...
    @PostMapping
    public ResponseEntity agregar(@RequestBody CuentasxcobrarDet c) {
        try {
    ...
    @PutMapping(path = {"/{id}"})
    public ResponseEntity editar(@RequestBody CuentasxcobrarDet c, @PathVariable("id") int id) {
        try {
            c.setId(id);
    ...
    @GetMapping(path = {"/findByCuentasxCobrar/{id_cuentasxcobrar}"})
    public ResponseEntity listarByIdCuentasxcobrar(@PathVariable("id_cuentasxcobrar") int id_cuentasxcobrar) {
        try {
            return ResponseEntity.ok(new ResponseDto(service.listarByIdCuentasxcobrar(id_cuentasxcobrar), true));
        } catch (Exception e) {
            LOG.error("No se pudo completar ", e);
            return ResponseEntity.internalServerError()
                    .body(new ResponseDto("No se pudo completar", false));
