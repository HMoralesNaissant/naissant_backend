...
public class SalesChannelsController {
    ...
    @Autowired
    SalesChannelsService service;

    private final Logger LOG = LoggerFactory.getLogger(SalesChannelsController.class);
    ...
    @PostMapping
    public ResponseEntity agregar(@RequestBody SalesChannels s) {
        try {
            return ResponseEntity.ok(new ResponseDto(service.add(s), true));
        } catch (Exception e) {
            LOG.error("No se pudo completar ", e);
            return ResponseEntity.internalServerError()
                    .body(new ResponseDto("No se pudo completar", false));
    ...
    @PutMapping(path = {"/{id}"})
    public ResponseEntity editar(@RequestBody SalesChannels s, @PathVariable("id") int id) {
        try {
            s.setId(id);
            return ResponseEntity.ok(new ResponseDto(service.edit(s), true));
        } catch (Exception e) {
            LOG.error("No se pudo completar ", e);
            return ResponseEntity.internalServerError()
                    .body(new ResponseDto("No se pudo completar", false));
    ...
    @GetMapping(path = {"/findByList/{id_list}"})
    public ResponseEntity listarByIdList(@PathVariable("id_list") int id_list) {
        try {
            return ResponseEntity.ok(new ResponseDto(service.listarByIdList(id_list), true));
        } catch (Exception e) {
            LOG.error("No se pudo completar ", e);
            return ResponseEntity.internalServerError()
                    .body(new ResponseDto("No se pudo completar", false));
