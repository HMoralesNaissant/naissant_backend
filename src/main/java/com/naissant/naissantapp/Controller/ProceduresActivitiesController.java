...
@RestController
@RequestMapping({"/proceduresactivities"})
@CrossOrigin(origins = "http://localhost:4200", maxAge = 3600)

public class ProceduresActivitiesController {

    @Autowired
    ProceduresActivitiesService service;

    private final Logger LOG = LoggerFactory.getLogger(ProceduresActivitiesController.class);
    ...
    @PostMapping
    public ResponseEntity agregar(@RequestBody ProceduresActivities p) {
        try {
    ...
    @PutMapping(path = {"/{id}"})
    public ResponseEntity editar(@RequestBody ProceduresActivities p, @PathVariable("id") int id) {
        try {
            p.setId(id);
    ...
    @GetMapping(path = {"/findByTypeProcedure/{type_procedure}"})
    public ResponseEntity listarByTypeProcedure(@PathVariable("type_procedure") String type_procedure) {
        try {
            return ResponseEntity.ok(new ResponseDto(service.listarByTypeProcedure(type_procedure), true));
        } catch (Exception e) {
            LOG.error("No se pudo completar ", e);
            return ResponseEntity.internalServerError()
                    .body(new ResponseDto("No se pudo completar", false));
