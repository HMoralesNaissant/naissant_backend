...
public class NivelEstudiosController {
    ...
    @Autowired
    NivelEstudiosService service;

    private final Logger LOG = LoggerFactory.getLogger(NivelEstudiosController.class);
    ...
    @PostMapping
    public ResponseEntity agregar(@RequestBody NivelEstudios n) {
        try {
            return ResponseEntity.ok(new ResponseDto(service.add(n), true));
        } catch (Exception e) {
            LOG.error("No se pudo completar ", e);
            return ResponseEntity.internalServerError()
                    .body(new ResponseDto("No se pudo completar", false));
    ...
    @PutMapping(path = {"/{id}"})
    public ResponseEntity editar(@RequestBody NivelEstudios n, @PathVariable("id") int id) {
        try {
            n.setId(id);
            return ResponseEntity.ok(new ResponseDto(service.edit(n), true));
        } catch (Exception e) {
            LOG.error("No se pudo completar ", e);
            return ResponseEntity.internalServerError()
                    .body(new ResponseDto("No se pudo completar", false));
