...
public class FranchisesController {
    ...
    @Autowired
    FranchisesService service;

    private final Logger LOG = LoggerFactory.getLogger(FranchisesController.class);
    ...
    @PostMapping
    public ResponseEntity agregar(@RequestBody Franchises f) {
        try {
    ...
    @PutMapping(path = {"/{id}"})
    public ResponseEntity editar(@RequestBody Franchises f, @PathVariable("id") int id) {
        try {
            f.setId(id);
