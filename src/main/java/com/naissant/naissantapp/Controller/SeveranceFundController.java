...
@RestController
@RequestMapping({"/severancefund"})
@CrossOrigin(origins = "http://localhost:4200", maxAge = 3600)

public class SeveranceFundController {

    @Autowired
    SeveranceFundService service;

    private final Logger LOG = LoggerFactory.getLogger(SeveranceFundController.class);
    ...
    @PostMapping
    public ResponseEntity agregar(@RequestBody SeveranceFund s) {
        try {
    ...
    @PutMapping(path = {"/{id}"})
    public ResponseEntity editar(@RequestBody SeveranceFund s, @PathVariable("id") int id) {
        try {
            s.setId(id);
